...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: platformAdmin } = await adminClient.from("platform_admin_users").select("auth_user_id").eq("auth_user_id", authData.user.id).eq("ativo", true).maybeSingle();
    ...
    const { data: empresa, error: empresaError } = await adminClient.from("empresas").select("id, slug, nome").eq("id", empresaId).maybeSingle();
    if (empresaError || !empresa) return response({ error: "Empresa nao encontrada." }, 404);
    await adminClient.from("company_first_access_invites").update({ revoked_at: new Date().toISOString() }).eq("empresa_id", empresaId).is("used_at", null).is("revoked_at", null);
    const rawToken = token();
    const { data: invite, error: inviteError } = await adminClient.from("company_first_access_invites").insert({ empresa_id: empresaId, token_hash: await hash(rawToken), created_by: authData.user.id }).select("id, expires_at").single();
    if (inviteError || !invite) return response({ error: inviteError?.message || "Convite nao criado." }, 400);
    const erpUrl = (Deno.env.get("MIKAON_ERP_URL") || "https://erp.mikaon.com.br").replace(/\/$/, "");
    return response({ data: { id: invite.id, empresaId: empresa.id, empresaNome: empresa.nome, expiresAt: invite.expires_at, url: `${erpUrl}/primeiro-acesso?token=${encodeURIComponent(rawToken)}` } });
